...
var NginxCacheKill = function(site_configs, default_config){
    ...
    this.createRedisClients = function(){
        ...
        var redisClient = new Spade( { socket: { address: { port: this.default_config.redis.port, host: this.default_config.redis.host}}});
        this.redisClient = redisClient;
        this.redisClient.on("connect", function(){
            logger.log('debug','redis connected'+redisClient);
        });
        this.redisClient.on('error', function(err){
            logger.log('error','redis error - '+err);
        });
        this.redisClient.cli(true);
    ...
    this.purgeRelated = function(url, cb){
        ...
        logger.log('debug','redisClient ready:'+this.redisClient.ready);
        var cachekill = this;
        this.redisClient.commands.smembers(url, function(is_err, data, reveal_fn){
            if (is_err){
                logger.log('error', 'redis smembers error:'+data);
                return;
            }
            var urls = reveal_fn(data);
            logger.log('debug','%s related urls found for:%s', urls.length, url);
            for (var i = 0; i < urls.length; i++) {
                cachekill.purgeUrl(urls[i], cb);
            }
        });
    }
