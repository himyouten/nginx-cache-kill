...
var NginxCacheKill = function(site_configs, default_config){
    ...
    this.createRedisClients = function(){
        ...
        this.redisClient.on("connect", function(){
            logger.log('debug','redis connected at %s:%s', redisClient.host, redisClient.port);
        });
    ...
    this.purgeUrl = function(url, cb){
        ...
        fs.unlink(filepath, function(err){
            ...
            if (typeof(cb)==='function') cb(err, url, filepath);
        });
    }
    ...
    this.purgeRelated = function(url, cb){
        logger.log('debug','purging related urls for:%s', url);
        logger.log('debug','redisClient:'+this.redisClient);
        var obj = this;
        this.redisClient.smembers(url, function(err, urls){
            ...
            for (var i = 0; i < urls.length; i++) {
                // purge the url, extend callback to remove from redis
                obj.purgeUrl(urls[i], function(cb_err, cb_url, cb_filepath){
                    // if error do not remove from redis
                    if (cb_err){
                        if (typeof(cb)==='function') cb(cb_err, cb_url, cb_filepath);
                    } else {
                        // remove the entry from redis
                        obj.redisClient.srem(url, cb_url, function(redis_err, redis_reply){
                            if (redis_err){
                                logger.log('error', "related url:%s for url:%s was not removed from datastore - " + redis_err, cb_url, url);
                            }
                            if (typeof(cb)==='function') cb(redis_err, cb_url, cb_filepath);
                        });
                    }
                });
            }
